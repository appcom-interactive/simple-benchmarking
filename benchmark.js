...
class Benchmark {
  static async executeSuite(testCases, args) {
    console.log(`Executing test suite '${args.name}'`);
  ...
  static displayExecutionTimeObjects(processes) {
    processes.forEach((proc, index) => {
      console.log(`${index + 1}.\n`);
      console.log(`Executed test case: '${proc.name}' in ${proc.rounds} rounds`);
  ...
  static async execute({ method, name = '', rounds = 100000, verbose = false } = {}) {
    const startTime = process.hrtime();
  ...
  static async measureExecution(method, args) {
    const startTime = process.hrtime();
    try {
      (await method);
    } catch (error) {
  ...
  static displayExecutionTime(time, title = 'Execution time:') {
    console.log(`${title}: ${time[0]}s ${(time[1] / 1000000)}ms`);
  };
